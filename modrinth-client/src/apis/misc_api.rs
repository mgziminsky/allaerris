...
impl MiscApi<'_> {
    ...
    pub async fn forge_updates(&self, params: &ForgeUpdatesParams<'_,>) -> Result<models::ForgeUpdates> {
        ...
        if local_var_status.is_client_error() || local_var_status.is_server_error() {
            ...
            let local_var_error = match local_var_status.as_u16() {
                400 => ForgeUpdatesError::Status400(serde_json::from_str(&local_var_content)?),
                _ => ForgeUpdatesError::Unknown(serde_json::from_str(&local_var_content)?),
            };
    ...
    pub async fn statistics(&self) -> Result<models::Statistics> {
        ...
        if local_var_status.is_client_error() || local_var_status.is_server_error() {
            #[allow(clippy::match_single_binding)]
            let local_var_error = match local_var_status.as_u16() {
                _ => StatisticsError::Unknown(serde_json::from_str(&local_var_content)?),
            };
            Err(ErrorResponse { status: local_var_status, content: local_var_content, source: Some(local_var_error.into()) }.into())
        } else {
            serde_json::from_str(&local_var_content).map_err(Into::into)
