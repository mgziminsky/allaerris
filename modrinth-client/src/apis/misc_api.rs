...
impl MiscApi<'_> {
    ...
    pub async fn forge_updates(&self, params: &ForgeUpdatesParams<'_,>) -> Result<models::ForgeUpdates> {
        ...
        if !status.is_client_error() && !status.is_server_error() {
            ...
            let error = match status.as_u16() {
                400 => ForgeUpdatesError::Status400(serde_json::from_str(&content)?),
                _ => ForgeUpdatesError::Unknown(serde_json::from_str(&content)?),
            };
    ...
    pub async fn statistics(&self) -> Result<models::Statistics> {
        ...
        if !status.is_client_error() && !status.is_server_error() {
            ...
            #[allow(clippy::match_single_binding)]
            let error = match status.as_u16() {
                _ => StatisticsError::Unknown(serde_json::from_str(&content)?),
            };
            Err(ErrorResponse { status, content, source: Some(error.into()) }.into())
        }
    }
