...
/// struct for passing parameters to the method [`MiscApi::forge_updates`]
#[derive(Clone, Debug)]
pub struct ForgeUpdatesParams<'l1,> {
...
/// struct for typed errors of method [`MiscApi::forge_updates`]
...
/// struct for typed errors of method [`MiscApi::statistics`]
#[derive(Debug, Clone, Serialize, Deserialize, thiserror::Error)]
#[serde(untagged)]
...
impl<'c> MiscApi<'c> {
    ...
    pub async fn forge_updates(&self, params: &ForgeUpdatesParams<'_,>) -> Result<models::ForgeUpdates> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity = serde_json::from_str::<ForgeUpdatesError>(&local_var_content).map(|e| Box::new(e) as _).ok();
    ...
    pub async fn statistics(&self) -> Result<models::Statistics> {
        #[allow(unused_mut)]
        let mut local_var_req_builder = self.0.request(
            reqwest::Method::GET,
            "/statistics"
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity = serde_json::from_str::<StatisticsError>(&local_var_content).map(|e| Box::new(e) as _).ok();
            let local_var_error = ErrorResponse { status: local_var_status, content: local_var_content, source: local_var_entity };
            Err(local_var_error.into())
        }
    }
