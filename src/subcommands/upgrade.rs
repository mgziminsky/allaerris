...
pub async fn get_platform_downloadables(
    ...
    let pad_len = profile.mods.iter().map(|m| m.name.len()).max().unwrap_or(20).clamp(20, 50);
    for mod_ in &profile.mods {
        ...
        tasks.spawn(async move {
            let _permit = permit;
            let result =
                get_latest_compatible_downloadable(&modrinth, &curseforge, &github, &mod_, &profile.game_version, profile.mod_loader).await;
            ...
            match result {
                Ok((downloadable, qf_flag)) => {
                    ...
                    {
                        to_download.lock().expect("Mutex poisoned").push(downloadable);
                        Ok((false, qf_flag))
                    }
                },
                Err(err) => {
                    if let mod_downloadable::Error::ModrinthError(ferinth::Error::RateLimitExceeded(_)) = err {
                        // Immediately fail if the rate limit has been exceeded
                        progress_bar.finish_and_clear();
                        bail!(err);
                    }
                    progress_bar.println(format!("{}", format!("{CROSS} {:pad_len$}  {err}", mod_.name).red()));
                    Ok((true, false))
                },
            }
        });
    ...
    Arc::try_unwrap(progress_bar)
        ...
        .finish_and_clear();
    if qf_flag {
        println!("{}", "Fabric mod using Quilt backwards compatibility".yellow());
    }
    Ok((
