    ...
    match &modpack.identifier {
        ModpackIdentifier::CurseForgeModpack(project_id) => {
            ...
            let manifest: Manifest = serde_json::from_str(
                &read_file_from_zip(modpack_file.try_clone().await.map(BufReader::new)?, "manifest.json")
                    .await?
        ...
        ModpackIdentifier::ModrinthModpack(project_id) => {
            ...
            let metadata: Metadata = serde_json::from_str(
                &read_file_from_zip(modpack_file.try_clone().await.map(BufReader::new)?, "modrinth.index.json")
                    .await?
