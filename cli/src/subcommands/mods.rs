...
use relibium::{
    ...
    Client, Config, ProfileManager,
};
use yansi::Paint;

use crate::{
    cli::{MgmtCommands, ModsSubcommand},
    helpers::{self, consts},
...
pub async fn process(subcommand: ModsSubcommand, config: &mut Config, client: Client) -> Result<()> {
    use MgmtCommands::*;
    use ModsSubcommand::*;
    let profile = helpers::get_active_profile(config)?;
    match subcommand {
        Add { ids, exclude } => {
            if ids.is_empty() {
