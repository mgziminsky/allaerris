...
impl ProfileManager {
    ...
    pub async fn install(&self, client: &Client, profile: &Profile) -> Result<Vec<LockedMod>> {
        ...
        if let Ok(errs) = delete_job.await {
            ...
            self.send_err(anyhow!("Unexpected error deleting old mods. The following files may need deleted manually:{files}",).into())
        }
